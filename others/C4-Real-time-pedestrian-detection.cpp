// C4-Real-time-pedestrian-detection.cpp : 定义控制台应用程序的入口点。
//
...
int main(int argc,char* argv[])
{
	...
	cv::Mat src;
	//cv::VideoCapture capture("rtsp://192.168.1.97:554/av0_1");
    cv::VideoCapture capture("../09_24_19.264");
	//cv::VideoCapture capture("E:\\2017-08-02\\1\\09_30_26.264");
	//cv::VideoCapture capture("D:\\3-Video of Pedestrians\\43.mp4");

	//cv::Rect detect_ROI(275, 325, 640, 215);		//960x540, 1	(275, 325, 600,215) , 2		(125, 300, 800,240), 3		(145, 120, 600,215)
	//cv::Rect detect_ROI(180, 290, 445, 180);       //640x480, 1	(180, 290, 445,180) , 2     (20, 80, 520,300)
	//cv::Rect detect_ROI(80, 50, 480, 320); 
	cv::Rect detect_ROI(0, 0, 640, 480);

	LoadCascade(scanner);
	std::cout<<"Detectors loaded."<<std::endl;
	int key = 0;
	int wait_time = 1;
	int step_size = 2;	// 2-->9,检测框步长由2改为9，检测速度大大的提高了
	float fx = 0.5;
	...
	while( key != 27 )
	{
		...
		cv::Mat detect_region = src(detect_ROI);
