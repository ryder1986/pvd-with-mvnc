...
public:
    ...
    bool Load(unsigned char* gray_img,int img_width,int img_height);
...
//}
template<class T>
bool IntImage<T>::Load(unsigned char* gray_img,int img_width,int img_height)
{
    SetSize(img_height, img_width);
    for(int i=0,ih=img_height,iw=img_width; i<ih; i++)
    {
        T* pdata = p[i];
        //unsigned char* pimg = reinterpret_cast<unsigned char*>(img.data+img.step*i);
        unsigned char* pimg = gray_img+img_width*i;
        for(int j=0; j<iw; j++)
        {
            pdata[j] = pimg[j];
        }
...
// The detector
DetectionScanner scanner(HUMAN_height,HUMAN_width,HUMAN_xdiv,HUMAN_ydiv,256,0.5);
...
void LoadCascade(DetectionScanner& ds)
{
    ...
    //upper_bounds.push_back(256);	//353
...
//将RGB转为Gray
//void RGB2Gray(int image_width, int image_height, Mat& rgb_image, unsigned char *gray_image)
//{
//	vector<Mat> channels;
//	split(rgb_image,channels);

//	for (int i = 0; i < image_height; ++i)
//	{
//		for (int j = 0; j < image_width; ++j)
//		{
//			unsigned char b = channels.at(0).at<uchar>(i,j);
//			unsigned char g = channels.at(0).at<uchar>(i,j);
//			unsigned char r = channels.at(0).at<uchar>(i,j);
//			gray_image[i * image_width + j] = (unsigned char)(r * 0.299 + g * 0.587 + b * 0.114);
//		}
...
//resize函数,最邻近插值
void resize_image(int src_image_width, int src_image_height, unsigned char *src_image, int dst_image_width, int dst_image_height, unsigned char *dst_image)
{
    float x_ratio = float(src_image_width) / dst_image_width;
    float y_ratio = float(src_image_height) / dst_image_height;

    for(int i = 0; i < dst_image_height; ++i)
    {
        for(int j = 0; j < dst_image_width; ++j)
        {
            int src_x = int(j * x_ratio + 0.5);
            int src_y = int(i * y_ratio + 0.5);
            dst_image[i * dst_image_width + j] = src_image[src_y * src_image_width + src_x];
        }
    }
}
#include <cstring>
//从原图像中截取选定的矩形区域
void get_image_rect(int src_image_width, int src_image_height, unsigned char *src_image, RECT region, unsigned char *dst_image)
{
    for(int i = region.y; i < src_image_height; ++i)
    {
        memcpy(&(dst_image[(i - region.y) * region.width]), &(src_image[i * src_image_width + region.x]), sizeof(unsigned char) * region.width);
    }
}
bool loaded=false;
int sec_start;
...
int test_img(  unsigned char*  small_image)
{
    //  cv::cvtColor(small_image,small_image,CV_BGR2GRAY);
    //cv::Mat img1(480,640,CV_8UC1,small_image) ;
...
//    cv::cvtColor(img1,img1,CV_BGR2GRAY);
  ...
  cout<<"1.13"<<endl;
    ...
    int key = 0;
    int wait_time = 1;
    int step_size = 9;	// 2-->9,检测框步长由2改为9，检测速度大大的提高了
  ...
  cout<<"1.14"<<endl;
    unsigned char* gray_image = new unsigned char[1920 * 1080];
    //  unsigned char* small_image = new unsigned char[640 * 480];
    unsigned char* detect_image = new unsigned char[region.width * region.height];
   time_old=cal_time();
    //while( key != 27 )
    {
        double start_time;
        ...
        //memcpy(temp_image, src.data, image_size);
        //	RGB2Gray(src.cols, src.rows, src, gray_image);

        //Mat temp_image(1080,1920,CV_8UC1,gray_image);
        //imshow("gray_image",temp_image);

        //    resize_image(src.cols,src.rows,gray_image,640,480,small_image);
        //get_image_rect(640,480,img1.data,region,detect_image);

        //cv::resize(src,src,Size(640,480));

        //   original.Load(detect_image,region.width,region.height);
        original.Load(small_image,region.width,region.height);
        std::vector<CRect> results;
        scanner.FastScan(original, results, step_size);
        cout<<"result number:"<<results.size()<<endl;
        ...
        cout<<time_now-time_old<<"ms"<<endl;
...
//         rectangle(img1, real_position, cv::Scalar(0,255,255), 2);
...
//        }
        ...
        //        {
        //            cv::Rect real_position;
        //            //real_position.x = results[i].left;//全图检测用的
        //            //real_position.y = results[i].top;//全图检测用的

        //            real_position = results[i].left + region.x;
        //            real_position.y = results[i].top + region.y;
        //            real_position.width = results[i].right - results[i].left;
        //            real_position.height = results[i].bottom - results[i].top;

        //        //	cv::rectangle(src, real_position, cv::Scalar(0,255,0), 2);

        //            result_num++;
        //            sprintf(result_name, "%d.jpg", result_num);
        //            cv::Mat result_image = src(real_position);
        //            imwrite(result_name,result_image);
        //        }

        //	cv::Rect detect_ROI(region.x,region.y,region.width,region.height);
        //	rectangle(src, detect_ROI, cv::Scalar(0,255,255), 2);
        //      //  cv::cvtColor(small_image,small_image,CV_BGR2GRAY);
        //  cv::Mat result_image(480,640,CV_8UC1,small_image) ;

        //         cv::cvtColor(result_image,result_image,CV_BGR2GRAY);
...
{
cout<<"1"<<endl;
    // /root/repo-github/project2/testserver/build-testvideosource-Desktop-Debug/test.y
    //  /media/211/videos/video-demo/8s-640x480-gop.yuv
   // FILE *f=fopen("/media/sf_D_DRIVE/repo-github/test.rgb","rb");
    FILE *f=fopen("./test.y","rb");
   // FILE *f=fopen("/root/repo-github/project2/testserver/build-testvideosource-Desktop-Debug/test.y","rb");
    // FILE *f=fopen("/media/211/videos/video-demo/8s-640x480-gop.yuv","rb");
    int pos=0;
    char buf[640*480];
    while(1){
        if(pos==90)
            pos=0;
        cout<<"1.1"<<endl;
        fseek(f,640*480*(pos++),SEEK_SET);
             cout<<"1.11"<<endl;
        fread(buf,640*480,1,f);
        // cout<<pos<<"read done"<<endl;
          cout<<"1.12"<<endl;
        test_img((unsigned char *)buf);
        cout<<"1.2"<<endl;
    }
cout<<"2"<<endl;
    //    char buf[640*480];
    ...
    //        if(pos==50)
    //            pos=0;
    //        fseek(f,640*480*3/2*(pos++),SEEK_SET);
    //        fread(buf,640*480,1,f);
    //       // cout<<pos<<"read done"<<endl;
    //        test_img((unsigned char *)buf);
    //    }
...
{
    cv::Mat src;
    cv::VideoCapture capture("../../09_30_26.264");
    ...
    while( key != 27 )
    {
        ...
        resize_image(src.cols,src.rows,gray_image,640,480,small_image);
        get_image_rect(640,480,small_image,region,detect_image);
        cv::resize(src,src,Size(640,480));
        ...
        for(size_t i = 0; i < results.size(); i++)
        {
            ...
            real_position.x = results[i].left + region.x;
            real_position.y = results[i].top + region.y;
            real_position.width = results[i].right - results[i].left;
            real_position.height = results[i].bottom - results[i].top;
